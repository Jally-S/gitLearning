  ...
  <body>
	<form name = "com">
		<fieldset title="查询">
			...
			客户名称：<input type="text" name="cname" class="input" style="width:250px; line-height:17px;display:inline-block" placeholder="请输入搜索关键字"/>
			客户爱好：<input type="text" name="hobby" class="input" style="width:250px; line-height:17px;display:inline-block" placeholder="请输入搜索关键字"/>
		...
		function query(num){
			...
			if(num == 1){//第一页
			 ...
			 $(document).ready(function(){
			 	//设置提交的路径，和参数
				$.post("<%=path%>/NeedQuery",{"cname":cname,"hobby":hobby,"pname":pname,"pageSize":pageSize,"pageCurrent":pageCurrent},
		...
		function controlButton(){//设置按钮可见与否，停在第一页时不能点击上一页。停在最后一页时，不能点击下一页
			...
			function goAdd(){
				...
				window.open("<%=path%>/manager/addNeed.jsp","新增对手",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
			}
			function goUpdate(cid){
				var width = window.screen.width ;
				var height = window.screen.height ;
				window.open("<%=path%>/FindNeedById?cid="+cid,"修改需求",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
			}
			...
			function goAddProduct(pid){
				var width = window.screen.width ;
				var height = window.screen.height ;
				window.open("<%=path%>/manager/addNeed.jsp?pid="+pid,"新增客户需求",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
			}
		</script>
</html>
