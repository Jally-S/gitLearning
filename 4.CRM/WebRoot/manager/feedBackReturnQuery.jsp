...
<html>
	...
	<body>
		<form name = "f">
			...
			维修回执单名称：
			<input type="text" name="feedName" class="input" style="width:250px; line-height:17px;display:inline-block" placeholder="请输入搜索关键字"/>
			产品名称：
			<input type="text" name="productName" class="input" style="width:250px; line-height:17px;display:inline-block" placeholder="请输入搜索关键字"/>
	...
	<script type="text/javascript">
		...
		function query(num){
			...
			$(document).ready(function(){//设置提交的路径，和参数
				$.post("<%=path %>/FeedBackReturnQuery",{"feedName":feedName,"productName":productName,"pageSize":pageSize,
		...
		function goUpdate(feedId){
			var width = window.screen.width;
			var height = window.screen.height;
			window.open("<%=path%>/FeedBackReturnFindByfeedId?feedId="+feedId,"修改维修回执单",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no' );
		}
		function goDelete(feedId){
			if(confirm("确认删除?")){
				 $(document).ready(function(){
			 	//设置提交的路径，和参数
				$.post("<%=path %>/FeedBackReturnDel",{"feedId":feedId},
				function(data){//Servlet执行完之后执行方法，data表示的servlet返回数据内容
	 				if(data == "true"){
	 					alert("删除成功");
	 					query(0);
