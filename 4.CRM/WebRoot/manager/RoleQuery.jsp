...
<html>
	...
	角色名字：<input type="text" name="RoleName" class="input" style="width:250px; line-height:17px;display:inline-block" placeholder="请输入搜索关键字"/>
	...
	<script type="text/javascript">
		...
		function query(num){
			...
			if(num == 1){
			 ...
			 $(document).ready(function(){
			 	//设置提交的路径，和参数
				$.post("<%=path%>/RoleQuery",{"RoleName":RoleName,"isDelete":isDelete,"pageSize":pageSize,"pageCurrent":pageCurrent},
		...
		function goAdd(){
			...
			window.open("RoleAdd.jsp","新增角色",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
		}
		function goUpdate(RoleId){
			var width = window.screen.width ;
			var height = window.screen.height ;
		
			window.open("<%=path%>/RoleFindByRoleIDId?RoleId="+RoleId,"修改角色",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
		}
