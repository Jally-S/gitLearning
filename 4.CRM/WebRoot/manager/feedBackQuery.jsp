...
<html>
	...
	<body>
		<form name = "f">
			...
			是否解决：<select name="isOver"class="input" style="width:70px; line-height:17px; display:inline-block">
	...
	<script type="text/javascript">
		...
		function query(num){
			...
			$(document).ready(function(){//设置提交的路径，和参数
				$.post("<%=path%>/FeedBackQuery",{"productName":productName,"isOver":isOver,"pageSize":pageSize,
		...
		function goAdd(feedBackId){
			var width = window.screen.width;
			var height = window.screen.height;
			window.open("<%=path%>/manager/feedBackReturnAdd.jsp?feedBackId="+feedBackId,"整改",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no' );
		}
