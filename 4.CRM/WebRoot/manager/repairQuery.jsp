	...
	<script type="text/javascript">
		...
		function query(num){
			...
			$(document).ready(function(){//设置提交的路径，和参数
				$.post("<%=path %>/QueryRepair",{"repairName":repairName,"isOver":isOver,"pageSize":pageSize,
		...
		function goAdd(repairId){
			var width = window.screen.width;
			var height = window.screen.height;
			window.open("<%=path%>/manager/repairReturnAdd.jsp?repairId="+repairId,"维修",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no' );
		}
