...
<head>
	...
	<body>
		...
		<fieldset title="查询">
			...
			<input type="hidden" name="customerId" value="${pojo.userId}">
	...
	<script type="text/javascript">
		...
		productName= f.productName.value;
			...
			// alert(customerId);
			 	//设置提交的路径，和参数
				$.post("<%=path %>/QueryFeedBack",{"productName":productName,"customerId":customerId,"pageSize":pageSize,"pageCurrent":pageCurrent},
		...
		function goAdd(){
			...
			window.open("addfeedback.jsp","新增反馈",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
		}
		function goUpdate(feedBackId){
			var width = window.screen.width ;
			var height = window.screen.height ;
			window.open("<%=path%>/FeedBackFindByCustId?feedBackId="+feedBackId,"修改反馈",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');		}
		
		function goDelete(feedBackId){
			if(confirm("确认删除？")){
				 $(document).ready(function(){
			 	//设置提交的路径，和参数
				$.post("<%=path %>/FeedBackDel",{"feedBackId":feedBackId},
				function(data){//Servlet执行完之后执行方法，data表示的servlet返回数据内容
	 				if(data == "true"){
	 					alert("删除成功");
	 					query(0);
