...
<html>
	...
	职工名称：<input type="text" name="workerName" class="input" style="width:250px; line-height:17px;display:inline-block" placeholder="请输入搜索关键字"/>
			
			在职状态：<select name="isDelete"  class="input" style="width:70px; line-height:17px; display:inline-block">
            ...
            <a class="button border-main icon-plus-square-o" onclick="goAdd()"> 添加职工</a>
		</fieldset>
	...
	<script type="text/javascript">
		...
		function query(num){
			...
			if(num == 1){
			 ...
			 $(document).ready(function(){
			 	//设置提交的路径，和参数
				$.post("<%=path %>/WorkerQuery",{"workerName":workerName,"isDelete":isDelete,"pageSize":pageSize,"pageCurrent":pageCurrent},
		...
		function goAdd(){
			...
			window.open("WorkerAdd.jsp","新增职工",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
		}
		function goUpdate(workerId){
			var width = window.screen.width ;
			var height = window.screen.height ;
		
			window.open("<%=path%>/WorkerFindByWorkerId?workerId="+workerId,"修改职工",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
		}
