	...
	<body>
		<form name="f">
			<fieldset title="查询">
				...
				人员姓名：
				<input type="text" name="userName" class="input" style="width:250px; line-height:17px;display:inline-block" placeholder="请输入搜索关键字"/>
...
function goIns(roleId,loginId) {
	$(document).ready(function() {
		$.post("<%=path%>/ManRoleIns", {
			"loginId" : loginId,
			"roleId" : roleId
		}, function(data) {
			if (data == "true") {
...
function goDel(roleId,loginId) {
		$(document).ready(function() {
			$.post("<%=path%>/ManRoleDel", {
				"loginId" : loginId,
				"roleId" : roleId
			}, function(data) {
				if (data == "true") {
