  ...
  <body>
	<form name = "com">
		<fieldset title="查询">
			...
			对手产品名称：<input type="text" name="pname" class="input" style="width:250px; line-height:17px;display:inline-block" placeholder="请输入搜索关键字"/>
			对手名称：<input type="text" name="cname" class="input" style="width:250px; line-height:17px;display:inline-block" placeholder="请输入搜索关键字"/>
			对手产品级别：<select name="clas" class="input" style="width:70px; line-height:17px; display:inline-block">
		...
		function query(num){
			...
			if(num == 1){//第一页
			 ...
			 $(document).ready(function(){
			 	//设置提交的路径，和参数
				$.post("<%=path%>/CompetitorProductQuery",{"pname":pname,"cname":cname,"clas":clas,"pageSize":pageSize,"pageCurrent":pageCurrent},
		...
		function controlButton(){//设置按钮可见与否，停在第一页时不能点击上一页。停在最后一页时，不能点击下一页
			...
			function goUpdate(cid){
				var width = window.screen.width ;
				var height = window.screen.height ;
				window.open("<%=path%>/FindCompetitorProductById?cid="+cid,"修改对手产品",'height=400,width=300,top='+(height-450)/2+',left='+(width-300)/2+',toolbar=no,menubar=no,scrollbars=no, resizable=no,location=no, status=no');
			}
			function goDelete(pid){
				...
				if(confirm("确认删除？")){
					 $(document).ready(function(){
				 	//设置提交的路径，和参数
					$.post("<%=path%>/DelCompetitorProduct",{"pid":pid},
					function(data){
